...
// gateStep: null (not gating) | 'code' | 'name'
...
// unmount itself mid-flow, before the optional name step ever showed.
export default function VerifyEmailGate({ children }) {
  ...
  useEffect(() => {
    if (gateStep === 'code' && !codeSent && session) {
      captchaRef.current
        ?.getToken()
  ...
  const handleSaveName = async (e) => {
    ...
    try {
      ...
      if (trimmed && trimmed !== profile?.full_name) {
        await updateProfileName(session.user.id, trimmed);
      }
      setGateStep(null);
    } catch (err) {
      setError(err.message);
      ...
      >
        ...
        {gateStep === 'name' && (
          <form onSubmit={handleSaveName}>
            ...
            <h2 className="font-display text-xl font-semibold text-white">What's your name?</h2>
            <p className="mt-2 text-sm leading-relaxed text-white/55">
              We got this from your Google account — change it if you'd like, or leave it as is.
            </p>
            <input
              type="text"
              autoFocus
